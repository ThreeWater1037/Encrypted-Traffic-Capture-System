...
- 新任务自动生成的 Master ID 与各 Worker 目录名完全一致，使用“任务名-16 位哈希”，支持中文；非法路径字符会替换，长名称会截短，同名任务仍生成不同标识。旧任务保留原映射，执行卡片和日志显示实际 Worker 目录名；API 显式指定的不安全目录 ID（如 Windows 保留名、末尾点或超长 UTF-8 名称）仍会转换为安全目录名。
