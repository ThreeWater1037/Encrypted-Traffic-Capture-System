...
class TargetCompletionLiveTests(unittest.TestCase):
    ...
    def test_stalled_iframe_does_not_fail_target_and_analytics_query_is_blocked(self):
        ...
        self.assertTrue(record.cleanup_summary["service"]["stopped"], record.cleanup_summary)
        self.assertIsNone(record.cleanup_summary["service"]["error"])
        self.assertIsNotNone(record.cleanup_summary["service"]["exit_code"])
