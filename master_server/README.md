...
Master 按 Worker 分别排队：不同 Worker 上的独立任务可以同时采集，同一 Worker
上的任务按提交顺序串行执行。一个任务涉及多台机器时，其中一台执行慢或离线重试，
不会阻塞其他机器继续处理后续任务。Worker 自身也保留单消费者采集队列，
Chrome、Edge、Firefox 使用相同的调度规则；不在同一 Worker 上同时启动多个采集任务。
尚未分发的任务读取日志时显示等待调度提示。

`limits.max_queue_size` 限制 Master 已接受且尚未结束的调度任务数（包含执行中的任务），
避免任务转入各 Worker 队列后绕过容量限制。重启恢复已持久化任务不受此上限阻挡，
并按创建顺序重新排队。
...
新建任务直接将 `job_id` 用作各 Worker 的 `task_id` 和目录名，不再按机器重新计算哈希。
自动生成的“任务名-16 位哈希”在前端和 Worker 上完全一致。同名新任务及重新开启的
新一轮仍生成独立 ID。历史任务始终读取已保存的 `worker_task_id`，不重命名现有目录。
API 显式指定的 ID 若为 Windows 保留名、末尾带点或 UTF-8 超过 240 字节，则转换为
安全目录名；前端执行卡片和日志展示实际映射。
