...
浏览器退出、驱动服务停止、TLS 密钥复制及临时配置清理分别计时。驱动服务退出超时后，
若强制终止并确认该驱动进程已退出，则记录清理警告，不否决已完成且必要文件有效的采集；
保留临时配置目录用于诊断（驱动退出不代表所有浏览器子进程均已退出）。驱动仍存活、
终止失败、其他退出异常、TLS 密钥复制失败等仍阻止检查点。清理警告和最终错误直接输出到
Worker 日志。完成检查点和网络状态文件还包含 `capture_summary` /
