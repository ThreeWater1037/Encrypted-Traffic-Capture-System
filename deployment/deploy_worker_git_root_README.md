...
每次部署在停止旧 Worker 后、新 Worker 启动前执行一次浏览器临时目录清理。
运行中的 Worker 仍每小时在任务间隙清理一次，不再保留 24 小时。
覆盖登记的 `wb_*`、`rust_mozprofile*` 和 Chrome/Edge 的下载、解包临时目录；
只检查系统临时目录第一层的明确名称，跳过仍有对应浏览器/驱动/更新进程、被占用、
其他用户所属或存在非预期链接的目录。不会清空整个 `/tmp` 或删除正式抓包产物。
